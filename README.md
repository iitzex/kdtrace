...
```bash
uv run main.py --cores 8
```

或是分析特定股票：

```bash
uv run main.py --sid 2330 --reload
```

## 📂 專案結構

- `main.py`: 系統入口，調度分析 (`StockAnalyzer`) 與視覺化 (`StockVisualizer`)。
- `crawl.py`: 股價爬取核心。
- `fetch.py`: 鉅亨網 (CNYES) 財報模型與資料抓取 (`CNYESFetcher`)。
- `indicator.py`: 技術指標 (KD, MA) 計算邏輯。
- `gen_html.py`: 現代化 HTML 報表生成器 (`HtmlGenerator`)。
- `util.py`: 集中化工具組（SSL 處理、路徑管理、CSV 讀取）。
- `del_wrong.py`: 靈活的資料裁切/回滾工具（支援前/後裁切）。
- `filter.py`: 基於基本面的股票篩選器。
- `rate.py`: 殖利率資料採集與 Excel 報表生成。

## ⚙️ 進階操作

### 資料裁切 (Data Trimming)
當資料發生錯誤需要回滾時，可使用 `del_wrong.py`：

```bash
# 刪除所有資料檔案的前 50 筆紀錄
uv run del_wrong.py --count 50 --pos first
```

### 篩選潛力股
執行 `filter.py` 根據預設條件（EPS 與營收成長）篩選股票：
