...
kdtrace/
├── src/                  # 所有 Python 程式碼
│   ├── main.py           # 主流程：讀 csv → fetch → KD/MA → 畫圖 → HTML
...
main.py     | 主分析流程 (Pool 批次)        | CLI      | fetch, indicator, gen_html, utils
crawl.py    | TWSE 股價爬蟲                 | CLI      | utils.http
fetch.py    | CNYES 財報 (5 endpoints)      | import   | utils.http
...
main.py        → 讀 data/{sid}.csv
               → fetch.py → json/{sid}_*.json (5 endpoints, 24h TTL)
               → indicator.kd/ma
               → matplotlib → pic/{sid}.png
               → gen_html.generate() → tse.html
```
...
ws.api.cnyes.com/…/quote               | fetch.py      | 現價 (column 21)
